  ...
  return (
    <main className="min-h-screen bg-white text-slate-900">
      ...
      >
        ...
        <div className="relative z-10 mx-auto flex min-h-[285px] max-w-[1400px] items-center px-6 py-7 sm:px-10 md:min-h-[295px] md:px-12 md:py-6 lg:px-16">
          <div className="w-full max-w-[700px]">
            <p className="text-2xl font-bold text-green-700 sm:text-3xl">
              Welcome to
            </p>

            <h1 className="mt-1 text-[3rem] font-extrabold leading-none tracking-tight text-slate-900 sm:text-5xl md:text-[4.25rem]">
              Meal Planner
            </h1>

            <p className="mt-4 max-w-[720px] text-lg leading-relaxed text-slate-800 sm:text-xl md:text-[1.1rem]">
        ...
        <div className="mx-auto grid max-w-[1320px] grid-cols-1 gap-5 md:grid-cols-4 md:gap-6">
          {steps.map((step, index) => (
            <div key={step.title} className="relative">
              <Link
                href={step.href}
                className={`group flex h-full min-h-[205px] w-full flex-col rounded-2xl border px-6 py-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md sm:min-h-[205px] sm:px-7 md:min-h-[225px] md:px-5 lg:px-6 xl:min-h-[235px] ${step.cardClass}`}
              >
                <div className="flex items-start justify-between">
                  ...
                  <div
                    className={`flex h-20 w-20 items-center justify-center rounded-full ${step.iconClass}`}
                  >
                    {step.icon}
                  </div>
                </div>
                ...
                <div className="mt-4 flex flex-1 flex-col">
                  <h2 className="text-2xl font-bold leading-tight text-slate-900 md:text-[1.3rem] lg:text-[1.45rem]">
                    {step.title}
                  </h2>

                  <p className="mt-2 text-base leading-relaxed text-slate-800 md:text-[0.9rem] lg:text-[0.95rem]">
                    {step.description}
                  </p>

                  <div
                    className={`mt-auto flex h-10 w-10 items-center justify-center self-end rounded-full text-2xl font-semibold text-white transition group-hover:translate-x-1 ${step.arrowClass}`}
                    aria-hidden="true"
                  >
                    →
                  </div>
              ...
              {index < steps.length - 1 && (
                <div
                  className="flex h-7 items-center justify-center text-3xl font-light text-green-700 sm:h-8 md:hidden"
                  aria-hidden="true"
                >
                  ↓
                </div>
