...
/*
 * Work out how many times each recipe appears
...
/*
 * Keep the Shopping List synchronised with
 * the Weekly Planner.
 */
function syncPlannerWithShoppingList(
  ...
  /*
   * Existing selected recipes that aren't known
   * to have come from the planner are treated as
   * direct/manual shopping-list selections.
   */
  ...
  /*
   * Calculate the number of times each recipe
   * currently appears in the planner.
   */
  const counts =
    getPlannerCounts(
      planner
  ...
  /*
   * Anything currently in the planner should
   * also be represented in the Shopping List.
   */
...
export default function WeeklyPlannerPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      /*
       * Make sure existing planner meals
       * are represented in the Shopping List.
       */
  ...
  useEffect(() => {
     ...
     * including recipe occurrence counts.
     */
    syncPlannerWithShoppingList(
      plannerMeals
    );
    ...
    /*
     * Tell Recipe Cards that the Planner changed.
     */
    window.dispatchEvent(
      new Event(
  ...
  /*
   * Clear every meal from the week.
   */
  function clearWeek() {
    const emptyPlanner =
      createEmptyPlanner();
  ...
  /*
   * While the saved planner is loading.
   */
  if (plannerMeals === null) {
    return (
                  ...
                  .map(
                    (recipe) => (
                      ...
                      <button
                        ...
                        className="w-full rounded-xl border border-gray-200 bg-white p-3 text-left transition hover:border-orange-400 hover:bg-orange-50"
                      >
